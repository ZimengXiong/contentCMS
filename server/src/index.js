...
async function ensureContentStructure() {
  ...
  if (!scriptExists) {
    throw new Error(
      "website.fish script not found. Expected at " + WEBSITE_SCRIPT
    );
  }
}
...
function runWebsiteScript(name) {
  return new Promise((resolve, reject) => {
    const child = spawn("fish", [WEBSITE_SCRIPT, "post", name], {
      cwd: CONTENT_ROOT,
    });
    let stderr = "";
    let stdout = "";
...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const file =
        req.files.file && Array.isArray(req.files.file)
          ? req.files.file[0]
          : req.files.file;
      const postDir = resolvePostDir(slug);
...
app.post("/api/deploy", async (req, res, next) => {
  try {
    ...
    // Run git commit
    await new Promise((resolve, reject) => {
      const gitCommit = spawn("git", ["commit", "-am", message], {
        cwd: hugoSiteDir,
      });
      let stderr = "";
      let stdout = "";
